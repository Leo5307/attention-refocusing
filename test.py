...
# def run(meta, config, starting_noise=None):
    ...
    # - - - - - prepare models - - - - - # 
# @torch.no_grad()
def run(meta,models,info_files, p, starting_noise=None,iter_id=0, img_id=0, save=True,image_size=[512,512],args=None):
    model, autoencoder, text_encoder, diffusion, config = models
    print("config", config['grounding_tokenizer_input'])
    ...
    if "input_image" in meta:
        ...
        input_image = F.pil_to_tensor( Image.open(meta["input_image"]).convert("RGB").resize((image_size)) ) 
    ...
    if save :
        ...
        for image_id, sample in zip(image_ids, samples_fake):
            ...
            sample = sample.resize(image_size)
            img2 = sample.copy()
...
def bbox_transform(bbox,image_size):
    bbox[0] = bbox[0] * 512/image_size[0]
    bbox[1] = bbox[1] * 512/image_size[1]
    bbox[2] = bbox[2] * 512/image_size[0]
    bbox[3] = bbox[3] * 512/image_size[1]
...
def main(args = None):
    ...
    #     print('o_boxes', o_boxes)   
        ...
        pp = 'a person with a hat'
        meta["prompt"] = pp
        o_names = ['hat', 'person']
        o_boxes = [[100,100,200,200], [50,200,250,500]]    
        width = 1280
        height = 720
        draw_bbox = deepcopy(o_boxes)
        print('o_boxes', o_boxes)
        for bbox in o_boxes:   
            bbox = bbox_transform(bbox, [width, height])
        print('o_boxes', draw_bbox)
            
        #number of generated images for one prompt
        i = 0
        for k in range(1):
            starting_noise = torch.randn(args.batch_size, 4, 64, 64).to(device) # random noise for the diffusion process
            p, ll  = format_box(o_names, o_boxes) #format box and phrases
            # print('p', p) #phrases
            print('ll', ll) #box
            l = np.array(o_boxes)
            # print('l', l)
            ...
            position, box_att = Pharse2idx_2(pp, name_box)  # box_att is the format box for CAR and SAR loss
            #save layout
            layout_folder = args.layout
            os.makedirs( layout_folder, exist_ok=True)
            draw_box_2(o_names, box_att ,layout_folder,str(i) + '_' +meta["prompt"].replace(' ',"_") + '.jpg',image_size=[width,height] )
            print('position', position )
            # phrase
            meta["phrases"] = p
            # location integer to visual box
            # meta['location_draw'] = l
            draw_bbox = np.array(draw_bbox)
            meta['location_draw'] = draw_bbox
            #location scale, the input GLIGEN
            meta["locations"] = l/512 # 512 is the size of the image(diffusion model input size)
            # the box format using for CAR and SAR loss
            meta['ll'] = box_att # box att is the formatted box for CAR and SAR loss
            # the locations of words which out of GPT4, label of boxes
            meta['position'] = position
            run(meta, models, info_files, args, starting_noise, k,i,image_size=[width,height],args=args)


if __name__ == "__main__":
    main()
    # bbox = generate_box_gpt4('a person with a hat')
    # print('bbox', bbox)
